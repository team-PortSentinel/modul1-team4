...
def get_top_vulnerabilities(limit: int = 10) -> dict[str, Any]:
    """team4_output.json의 전체 결과에서 상위 취약점을 조회한다."""
    ...
    return {
        "total_count": len(RISK_RECORDS),
        "returned_count": len(results),
        "max_limit": 50,
        "results": results,
        "source": "team4_output.json",
    }
...
TOOLS = [
    ...
    },
    {
        ...
        "description": (
            "team4_output.json의 전체 위험도 결과에서 "
            "상위 취약점 순위를 조회한다. "
...
- 대화 기록의 CVE와 team4_output.json의 결과가 다르면
  team4_output.json 결과를 우선합니다.
답변 원칙:
